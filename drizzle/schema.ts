...
export const users = mysqlTable("users", {
  ...
  role: mysqlEnum("role", ["user", "technician", "admin"]).default("user").notNull(),
...
export const tickets = mysqlTable("tickets", {
  ...
  estimatedResolutionTime: varchar("estimatedResolutionTime", { length: 100 }),
  actualResolutionTime: decimal("actualResolutionTime", { precision: 10, scale: 2 }),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
...
export const ticketHistory = mysqlTable("ticket_history", {
  ...
  changeType: mysqlEnum("changeType", ["created", "status_changed", "assigned", "priority_changed", "category_changed", "comment_added", "attachment_added", "other"]).notNull(),
  description: text("description"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, (table) => ({
  ticketIdx: index("ticketId_idx").on(table.ticketId),
...
export const notifications = mysqlTable("notifications", {
  ...
  type: mysqlEnum("type", ["ticket_created", "ticket_assigned", "ticket_status_changed", "comment_added", "attachment_added", "ticket_resolved", "general"]).notNull(),
...
export const ticketsRelations = relations(tickets, ({ one, many }) => ({
  ...
  createdBy: one(users, { fields: [tickets.createdByUserId], references: [users.id], relationName: "createdTickets" }),
  assignedTo: one(users, { fields: [tickets.assignedToUserId], references: [users.id], relationName: "assignedTickets" }),
  comments: many(ticketComments),
  attachments: many(attachments),
...
export const ticketHistoryRelations = relations(ticketHistory, ({ one }) => ({
  ...
  changedBy: one(users, { fields: [ticketHistory.changedByUserId], references: [users.id] }),
}));
